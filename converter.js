...
var SOCKET_IO_SERVER_HOST = "172.16.2.85";	// ホスト名
...
csvParse(com_config, { comment: '#' }, function (err, csvOutput) {
	...
	serialport_name = csvOutput[0][0] + "";	// シリアルポート名

	serialport_propery = {			// シリアル基本設定
		baudRate: Number(csvOutput[0][2]),
		dataBits: 8,
		parity: 'none',
		stopBits: 1,
		flowControl: true,
	...
	SOCKET_IO_SERVER_HOST = csvOutput[0][1];
	...
	Serialport.list(function (err, ports) {
		...
		if (ports.length > 0) {
			...
			serial = new Serialport(serialport_name, serialport_propery);
