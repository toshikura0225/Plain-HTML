...
var jquery_src = fs.readFileSync('./jquery.js');
var style_src = fs.readFileSync('./style.css');
...
var app = http.createServer(function (req, res) {
	...
	else if (url_parts.pathname == '/jquery.js') {
		res.writeHead(200, { 'Content-Type': 'text/javascript' });
		res.write(jquery_src);
		res.end();
	}
...
io.sockets.on('connection', function(socket) {
	...
	socket.on('path-through', function(data) {
		...
		socket.emit('path-through', data);	// 全員の応答
	});
